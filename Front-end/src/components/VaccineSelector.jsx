...
import { Search, Syringe, ChevronLeft, ChevronRight, Loader2, X, Plus, DollarSign, CheckCircle2 } from 'lucide-react';
import { useSelector } from 'react-redux';

const VaccineSelector = React.forwardRef(({ selectedVaccines, onSelectVaccine, onRemoveVaccine }, ref) => {
                ...
                {vaccines.map((v) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => !isSelected && onSelectVaccine(v)}
                    >
            ...
            {selectedVaccines.length > 0 ? (
              <div className="divide-y divide-gray-800/50">
                {selectedVaccines.map((v) => (
                  <div key={v.id} className="p-3 flex justify-between items-center animate-in slide-in-from-right-2 duration-200">
                    <div className="flex-1 min-w-0 pr-4">
                      <h4 className="text-sm font-medium text-white truncate">{v.name}</h4>
                      <p className="text-[12px] text-dark-primary font-semibold">{v.price.toLocaleString('vi-VN')} VNĐ</p>
                    </div>
                    <button
                      onClick={() => onRemoveVaccine(v.id)}
